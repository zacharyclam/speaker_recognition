...
if __name__ == '__main__':
    ...
    config.gpu_options.per_process_gpu_memory_fraction = 0.42  # 占用GPU90%的显存
    K.set_session(tf.Session(config=config))
    model = Sequential()

    model.add(Conv1D(filters=16, kernel_size=5, strides=2, activation='relu', input_shape=(11960, 1), name='block1_conv1'))
    model.add(Conv1D(filters=16, kernel_size=3, strides=1, activation='relu', name='block1_conv2'))
